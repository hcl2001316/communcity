...
public class MessageController implements CommunityConstant {
    ...
    public String getLetterDetail(@PathVariable("conversationId") String conversationId, Page page, Model model) {
        ...
        List<Integer> letterIds = getLetterIds(letterList);//未读的id列表
        messageService.readMessage(letterIds);   //将这些消息设置为已读
    ...
    public String sendLetter(String toName, String content) {
        User target = userService.selectByName(toName);     //通过接收者的名字查询到接收者
        if (target == null) {
            return CommunityUtil.getJSONString(1, "目标用户不存在!");
        }
        Message message = new Message();
        message.setFromId(hostHolder.getUser().getId());   //发送者就是自己
        message.setToId(target.getId());       //设置接收者
        if (message.getFromId() < message.getToId()) {     //拼接
    ...
    public String getNoticeDetail(@PathVariable("topic") String topic, Page page, Model model) {
        ...
        if (noticeList != null) {
            for (Message notice : noticeList) {
                Map<String, Object> map = new HashMap<>();
                // 通知
                map.put("notice", notice);    //这里面的content格式是乱的
                // 内容
                String content = HtmlUtils.htmlUnescape(notice.getContent());   //去掉转移字符
